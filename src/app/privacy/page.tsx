      ...
      <main className="wrap page">
        ...
        <div className="prose">
          <p className="notice notice--warn">
            <strong>公開前に必ずご確認ください。</strong>{"このページは雛形です。運営者名・連絡先・利用する解析ツールと広告配信の有無を確定させ、内容を確認したうえで公開してください。"}
            {"とくに広告配信（Google AdSense等）を導入する場合、パーソナライズ広告に関する記載が必要になります。"}</p>

          <h2>1. 取得する情報</h2>
          <p>{"当サイトは、お問い合わせフォームまたはメールでご連絡いただいた場合に、お名前・メールアドレス・本文に記載された情報を取得します。"}
            {"また、アクセス解析のために、閲覧ページ・参照元・端末やブラウザの種類などの統計情報を取得することがあります。"}</p>
          ...
          <p>{"当サイトは、アクセス状況の把握のためにアクセス解析ツールを利用することがあります。解析ツールはCookieを使用する場合がありますが、個人を特定する情報は含みません。"}
            {"Cookieの利用はブラウザの設定で拒否できます。"}</p>

          <h2>4. 広告について</h2>
          <p>{"当サイトは、広告配信サービスを利用する場合があります。広告配信事業者がCookie等を使用して、利用者の興味関心に応じた広告を表示することがあります。"}
            {"この場合、当サイトは個人を特定できる情報を広告配信事業者へ提供しません。"}</p>
          <p className="muted">
